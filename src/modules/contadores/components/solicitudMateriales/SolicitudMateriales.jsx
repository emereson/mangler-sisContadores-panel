...
import TablaSolicitudMateriales from "./components/TablaSolicitudMateriales";
...
const SolicitudMateriales = ({ userData }) => {
  ...
  const exportToExcel = () => {
    ...
    const data = pedidos?.flatMap((pedido) =>
      pedido.lista_pedidos.map((lista) => ({
        ...
        COD_MAT: lista.materiale.codigo,
        MATERIAL: lista.materiale.descripcion,
        SOLICITADO: lista.cantidad,
        ATENDIDO: lista.cantidad,
      }))
    );
  ...
  return (
    <div className="w-full  h-full p-4 flex flex-col gap-3">
      <h2 className="font-semibold text-lg">Solicitud Materiales</h2>
      ...
      <TablaSolicitudMateriales pedidos={pedidos} />
    </div>
  );
};
